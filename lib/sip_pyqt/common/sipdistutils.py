...
class build_ext (build_ext_base):
    ...
    def build_extension (self, ext):
        ...
        # The compiler by default starts with the arguments used when compiling Python itself
        # This means that we may get a configuration thought for C, that we have to patch
        # (actually, the fact that we get a C++ compiler and not a C one is accidental; see
        ...
        try:
            ...
            if c.compiler_type=='unix':
                ...
                if not c.initialized:
                    c.initialize()
                # On VC++, if we do not specify /EHsc the C++ exception semantics is broken
                # (destruction on stack unwinding is not performed correctly, and catch(...)
                # eats also Win32 SEH exceptions)
                ehOptions=[opt for opt in c.compile_options if opt.startswith('/EH')]
                if '/EHsc' not in c.compile_options:
                    if len(ehOptions):
                        # We already have some unexpected /EH... option around
                        print 'WARNING: strange exception semantics specified in compile_options:', ehOptions
                    else:
